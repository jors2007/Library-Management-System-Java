...
public class Staff extends Users {
    ...
    public boolean CheckOutItem(int book_id, Users current_borrower, ArrayList<Books> BooksList, Loan Current_Loan, ArrayList<Loan> LoanList) {
        ...
        if (!(current_borrower instanceof LoanableUser)) {
            System.out.println("User is not loan-capable");
            return false;
        }
        LoanableUser lu = (LoanableUser) current_borrower;

        if (lu.GetFineStatus() == true) {
...
//        while (itr.hasPrevious()) {
          ...
          //  Loan L = itr.previous();
            ...
            if ((L.GetaBookId() == book_id) && (L.GetaBorrowerId() == current_borrower.GetId()) && L.GetStatus()==false) {
                ...
                if (user_fine > 0.0) {
                    ...
                    lu.SetFineStatus(true);
                    lu.SetFineAmount(user_fine);

                    RenewItem(book_id, BooksList, 1);
                    lu.UpdateLoanInfo(L, book_id);
                    return true;
    ...
    public String ViewInformation() {
        ...
        for ( int i=0; i <LoanList.size(); i++) {
            ...
            if (/*(l.GetStatus() == false) &&*/ currentborrower == this.GetId()) {
          ...
          for (int counter=LoanList.size()-1 ; counter >=0; counter--){
            ...
            if (L.GetaBorrowerId() == user_id && F.equals("Fined")==true ){
                ...
                for (Users b : BorrowerLists) {
                    ...
                    if (b.GetId() == user_id) {
                        if (b instanceof LoanableUser) {
                            LoanableUser lu = (LoanableUser) b;
                            lu.SetFineAmount(0);
                            lu.SetFineStatus(false);
                            lu.UpdateLoanInfo(L, book_id);
                        }
                        break;
                    } 
