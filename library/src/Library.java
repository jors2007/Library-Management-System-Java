...
public class Library {
    ...
    void LibrarianAddNewBook( String NewAuthor ,String NewTitle ,String NewSubject , int quantity , int lib_id)
    {
          for (int i = 0; i < LibrarianList.size(); i++) {
            ...
            Librarian L = LibrarianList.get(i);
            if (L.GetId() == lib_id) {
                // Call through the ManageBooks interface to respect LSP
                if (L instanceof ManageBooks) {
                    ManageBooks mgr = (ManageBooks) L;
                    mgr.AddBook(BooksList, NewAuthor, NewTitle, NewSubject, quantity);
                }
            }
        ...
        boolean result=false;
         ...
         for (int i = 0; i < LibrarianList.size(); i++) {
            ...
            if (L.GetId() == lib_id) {
               if (L instanceof ManageBooks) {
                   ManageBooks mgr = (ManageBooks) L;
                   result = mgr.DeleteBook(BooksList, book_id);
               }
            }
    ...
    void LibrarianUpdateBookInfo(int book_id , int lib_id , String NewInfo, int newquantity, int command)
    {
        ...
        for (int i = 0; i < LibrarianList.size(); i++) {
            ...
            if (L.GetId() == lib_id) {
                if (L instanceof ManageBooks) {
                    ManageBooks mgr = (ManageBooks) L;
                    mgr.ChangeInfo(BooksList, book_id, command, NewInfo, newquantity);
                }
            }
